    ...
    if len(args.nb_classes) == 3:
        for samples, segments, targets, family_targets, mf_targets in metric_logger.log_every(data_loader, print_freq, header):
            ...
            with torch.cuda.amp.autocast():
                outputs, family_out, manu_out, ortho_loss = model(samples, segments)
                if not args.cosub: 
                    ...
                    if args.globalkl:
                        ...
                        loss = loss + gk_loss * args.gk_weight+ ortho_loss * args.ortho_weight
                else:
                    ...
                    loss = loss + ortho_loss * args.ortho_weight
            ...
            metric_logger.update(ortho_loss=ortho_loss.item())
        ...
        for samples, segments, targets, family_targets in metric_logger.log_every(data_loader, print_freq, header):
            ...
            with torch.cuda.amp.autocast():
                outputs, family_out , ortho_loss = model(samples, segments)
                if not args.cosub:
                    loss_species = criterion(samples, outputs.float(), targets)
                    loss_family = criterion(samples, family_out, family_targets)
...
def evaluate(data_loader, model, device, nb_classes):
    ...
    if len(nb_classes) == 3:
        ...
        for images, segments, target, family_targets in metric_logger.log_every(data_loader, 10, header):
            ...
            # compute output
            with torch.cuda.amp.autocast():
                output, family_out,_ = model(images, segments)
                loss_species = criterion(output, target)
                loss_family = criterion(family_out, family_targets)
            ...
            family_acc1, = accuracy(family_out, family_targets, topk=(1,))
            # family_acc5 = torch.tensor([100.0], device=family_out.device)
