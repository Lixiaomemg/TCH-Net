...
@torch.no_grad()
def evaluate_detail(data_loader, model, device, filename, nb_classes,output_dir, dataset='AIR-SUPERPIXEL', breeds_sort=None):
    ...
    if len(nb_classes) == 3:
        ...
        for images, segments, target, family_targets, mf_targets in metric_logger.log_every(data_loader, 1, header):
            ...
            with torch.cuda.amp.autocast():
                output, family_out, manu_out,_ = model(images, segments)#=====================test===================
            ...
            family_acc1,  = accuracy(family_out, family_targets, topk=(1, ))
            manu_acc1,  = accuracy(manu_out, mf_targets, topk=(1, ))
        ...
        stats_line1 = '* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} family@1 {familytop1.global_avg:.3f}' \
                      ' manu@1 {manutop1.global_avg:.3f} sploss {losses.global_avg:.3f} fmloss {fmlosses.global_avg:.3f} mfloss {mflosses.global_avg:.3f}'\
                      .format(top1=metric_logger.acc1, top5=metric_logger.acc5, losses=metric_logger.sploss, fmlosses=metric_logger.famloss, mflosses=metric_logger.manuloss,
                      familytop1=metric_logger.family_acc1, manutop1=metric_logger.manu_acc1)
    
    elif len(nb_classes) == 2:
        trees = json.load(open('data/'+dataset + '_tree.json'))
        ...
        for images, segments, target, family_targets in metric_logger.log_every(data_loader, 1, header):
            ...
            # compute output
            with torch.cuda.amp.autocast():
                output, family_out,_ = model(images, segments)#=====================test===================
                loss_species = criterion(output, target)
                loss_family = criterion(family_out, family_targets)
        ...
        stats_line1 = '* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} family@1 {familytop1.global_avg:.3f}' \
                  ' sploss {losses.global_avg:.3f} fmloss {fmlosses.global_avg:.3f} ' \
                  .format(top1=metric_logger.acc1, top5=metric_logger.acc5, losses=metric_logger.sploss, 
                          fmlosses=metric_logger.famloss, familytop1=metric_logger.family_acc1)

    # print(f"FPA: {(fpa_cnt / total_cnt) * 100:.3f}% | TICE: {((total_cnt - tice_cnt) / total_cnt) * 100:.3f}% ")
    stats_line2 = f"FPA: {(fpa_cnt / total_cnt) * 100:.3f}% | TICE: {((total_cnt - tice_cnt) / total_cnt) * 100:.3f}% "
    stats_output = stats_line1 + "\n" + stats_line2 + "\n"
    print(stats_output)
    with (output_dir / "output.txt").open("a") as f:
        f.write(json.dumps(stats_output) + "\n")
