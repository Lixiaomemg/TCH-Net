...
def create_optimizer(args, model, filter_bias_and_bn=True):
    # 只需要返回参数组、优化器名称和 kwargs
    return model, create_optimizer_from_groups, args.opt, optimizer_kwargs(cfg=args)
def create_optimizer_from_groups(
        optimizer_name: str,
        param_groups: list, # 接收我们自定义的参数组列表
        momentum: float = 0.9,
        **kwargs):
    
    opt_lower = optimizer_name.lower()
    
    # 核心：将参数组列表 (param_groups) 传递给优化器
    parameters = param_groups

    opt_args = dict(**kwargs)

    # 确保不向优化器传入与参数组冲突的全局 LR/WD
    opt_args.pop('lr', None)
    opt_args.pop('weight_decay', None)
    
    # --- 根据 opt_lower 创建优化器 (逻辑与原 create_optimizer_v2 保持一致) ---
